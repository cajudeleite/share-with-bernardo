...
class Seller:
    ...
    def get_sales(self):
        ...
        df['price'] = df['price'].astype(
            float)  # Convert the 'price' column to float
        sales_df = df.groupby('seller_id')['price'].sum().reset_index(
        )  # Group by 'seller_id' and sum the 'price' column
        sales_df = sales_df.rename(
            columns={'price': 'sales'})  # Rename the 'price' column to 'sales'
        return sales_df
    ...
    def get_review_score(self):
        ...
        sellers = df.sort_values(by="seller_id").copy()
        sellers["review_cost"] = [
            score_to_cost(score) for score in sellers["review_score"]
        ]
        seller_cost_sum = sellers.groupby(
            "seller_id")["review_cost"].sum().reset_index()
        ...
        res["reviews_cost"] = seller_cost_sum["review_cost"]
